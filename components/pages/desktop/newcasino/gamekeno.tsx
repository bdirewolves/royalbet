...
export default function GameKeno(){
    ...
    const fetchProviderCard = async () => {
        try {
            const tmp = await axios.get(`${process.env.API_URL}/partner/providers/keno`).then((res) => res.data.data)
            const slots = tmp.filter((item: IProvider) => item.type == "keno")
            setProviders(slots)
        } catch (error) {
            Swal.fire({
                title: "Error",
            ...
            <GridBox>
                {
                    providers.map((item , index) => (
                        <Bigbox key={index} namegame={item.name} imggame={`assets/img/icon/providers/keno/${item.name}.png`}/>
                    ))
                }
            </GridBox>
        </ContainerGameSection>
